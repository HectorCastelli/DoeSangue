		...
		<div data-role="content">
			...
			if (mysql_connect($server,$username,$password)) {
				...
				if (mysql_select_db($database)) {
					...
					$query = mysql_query('SELECT id, nome, dia, num_vagas, hora_inicio, hora_final,gestor,nome FROM campanhas_gestores WHERE gestor_id = "'.$_GET['f'].'"');
					if ($_GET['i'] == 0) {
						$query = mysql_query('SELECT id, nome, dia, num_vagas, hora_inicio, hora_final, nome, gestor FROM campanhas_gestores');
					}
					if ($query == "") {
						//nulo
						...
						echo '<label for="campanha" class="select">Selecione sua campanha</label>';
