...
<body>
			...
			if (mysql_connect($server,$username,$password)) {
				...
				if (mysql_select_db($database)) {
					...
					if ($query == "") {
						...
						echo'<div align="center" class="page-header"><ul class="pager"></ul><font size="8"><span class="label label-danger">Doe Sangue</span> <br> Selecione o melhor horário para você: </font></div>';
						echo '<nav class="navbar navbar-default navbar-fixed-top"><br><div class="progress"><div class="progress-bar progress-bar-danger progress-bar-striped active" role="progressbar" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100" style="width: 50%;">Horário<span class="sr-only"></span></div></div></nav>';
						echo '<div class="jumbotron">';
						for ($i = 1; $i<$cont; ++$i) {
							...
							if ($query == "") {
								...
								if ($result[0] < $vagas) {
									...
									echo '<div class="input-group" align="center"><span style="border:none;" class="input-group-addon"><input  required type="radio" aria-label="..." name="radio-choice" id="radio-choice-'.$i.'" value="'.$i.'" />';
									...
									echo '<div class="input-group" align="center"><span style="border:none;" class="input-group-addon"><input disabled  type="radio" aria-label="..." name="radio-choice" id="radio-choice-'.$i.'" value="'.$i.'" />';
									echo '<label for="radio-choice-'.$i.'"><H4><span class="label label-danger">'.date('H:i	',$hora_atual).'</span><span class="badge">'.($vagas-$result[0]).' vagas livres.</label></span></H4></span></div><br>';
								}
							}
							$hora_atual += 1200;
						}
	...
	<nav class="navbar navbar-default navbar-fixed-bottom">
			...
			<div class="btn-group btn-group-justified" role="group" aria-label="...">
					...
					<?php	if ($cidade==NULL){
									echo '<a href="index.php" class="btn btn-danger"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span>           Voltar</a>';
								}else{
									//comando para que o comando voltar seja efetivo
									echo '<a href="cidades.php?cidade='.$cidade.'"  class="btn btn-danger"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span> Voltar</a>';
								}?>	
					</div>	
				</div>
			<br>	
